(function() {
  ...
  var CONTENT_DELIVERY_URL =
      "https://content-delivery." + "istex" + ".fr" + "/web-header", // split istex.fr because of ezproxy automatic parsing (istex-view use case)
  ...
  function init($) {
    ...
    if (
      (window.localStorage &&
        window.localStorage.getItem("web_header_local")) ||
      (window.location.search &&
        window.location.search.match(/web_header_local(=true)?(&|$)/)) ||
    ...
    var prependToTarget =
      $("script[data-header-target]").data("headerTarget") || "body";
      ...
      success: function(data) {
        ...
        $.ajax({
          ...
          success: function(_data) {
            ...
            var $webHeader = $(jQuery.parseHTML(_data))
              ...
              .end()
              .find('[href="' + window.location.href + '"]')
              .addClass("disabled")
              .click(preventDefaultEvent)
              .end();
            window.location.hostname === "www.istex.fr" &&
              $webHeader.find(".logoistex").remove();
          },
